...
class PCNReportWizard(models.TransientModel):
    ...
    def _get_header_entry(self):
        ...
        journal_entry_filters = self.env['account.move'].search([
            ('state', '=', 'posted'),
        ...
        # filter by dates:
        entries_in_range = journal_entry_filters.filtered(lambda move: (
                (
        ...
        for entry in entries_in_range:
            ...
            if entry.move_type in ['in_invoice', 'in_refund'] and entry.pcn_codes == 'T':
                ...
                if taxed_amount:
                    ...
                    amount_untaxed = round(entry.amount_total - amount_taxed)
